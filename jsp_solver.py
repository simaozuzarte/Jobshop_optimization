"""
Job Shop Scheduling Problem (JSP) Solver
Main interface that coordinates MIP and CP-SAT solvers
"""
...
def parse_arguments():
    ...
    parser = argparse.ArgumentParser(
        description="Job Shop Scheduling Problem (JSP) Solver",
...
def main():
    ...
    try:
        ...
        for idx, inst in enumerate(instances, 1):
            ...
            # Read instance data once
            n_jobs, n_machines, jobs, p = read_jsplib_instance(inst)
            instance_info = get_instance_optimum(inst, optimum_data)
            optimum = instance_info.get('optimum')
            ...
            if args.solver in ["mip", "both"]:
                ...
                if mip_res.get("schedule"):
                    ...
                    mip_res["schedule_valid"] = is_valid
                    mip_res["validation_violations"] = violations if not is_valid else None
                    if not is_valid:
